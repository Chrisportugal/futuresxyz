    ...
    <div className="trade-panel">
      ...
      {showLevInput && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <input type="range" className="tp-slider" min={1} max={maxLev} value={leverage}
            onChange={e => { setLeverage(parseInt(e.target.value)); setLevInput(e.target.value) }} />
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <div className="trade-leverage-labels" style={{ flex: 1 }}><span>1x</span><span>{maxLev}x</span></div>
            <div className="trade-input-wrapper" style={{ width: 70, padding: '4px 8px' }}>
              <input type="number" className="trade-input" style={{ fontSize: 13 }} value={levInput}
      ...
      <div className="tp-info-rows">
        ...
        <div className="tp-info-row">
          ...
          <span style={{ color: posSize > 0 ? 'var(--green)' : posSize < 0 ? 'var(--red)' : undefined }}>
            {posSize !== 0 ? `${posSize.toFixed(5)} ${coin}` : `0.00000 ${coin}`}
          </span>
        </div>
      </div>
      ...
      <div className="trade-input-group">
        <div className="trade-input-wrapper">
          ...
          <input type="number" className="trade-input" placeholder="0.00" value={size}
            onChange={e => {
      ...
      {!isConnected ? (
        ...
        <button className="trade-submit transfer" onClick={async () => { switchToArbitrum(); setTimeout(approveAgent, 2000) }} disabled={approving}>
        ...
        <button
          ...
          onClick={() => { if (!size || tokenSize <= 0) return; setShowConfirm(true) }}
      ...
      <div className="tp-summary">
        ...
        <div className="tp-summary-row">
          ...
          <span>{orderValue > 0 ? `${formatUsd(orderValue)} USDC` : 'N/A'}</span>
        </div>
        <div className="tp-summary-row">
          <span>Margin Required</span>
          <span>{orderValue > 0 ? `${formatUsd(orderValue / leverage)} USDC` : 'N/A'}</span>
        </div>
        <div className="tp-summary-row">
          <span>Slippage</span>
          <span>{orderType === 'market' ? 'Est: 0.10% / Max: 3%' : '--'}</span>
        </div>
        <div className="tp-summary-row">
          <span>Fees</span>
          <span>0.0350% / 0.0150%</span>
        </div>
      </div>
      ...
      {showConfirm && (
        <OrderConfirmModal
          market={selectedMarket} side={side}
          size={tokenSize.toFixed(market?.szDecimals ?? 4)}
          price={priceNum} leverage={leverage} orderType={orderType}
          onCancel={() => setShowConfirm(false)}
          onConfirm={() => { setShowConfirm(false); handleSubmit() }}
        />
      )}

      {/* Deposit — full width like Hyperliquid */}
      {isConnected && (
        <button className="tp-hl-deposit" onClick={() => window.open('https://app.hyperunit.xyz/deposit', '_blank')}>
          Deposit
        </button>
      )}

      {/* Perps <-> Spot | Withdraw — side by side like Hyperliquid */}
      {isConnected && (
        <div className="tp-hl-actions">
          <button className="tp-hl-action-btn" onClick={() => {
            if (exchange) exchange.usdClassTransfer({ amount: 0, toPerp: true }).catch(() => {})
          }}>
            Perps &#x21C4; Spot
          </button>
          <button className="tp-hl-action-btn" onClick={() => window.open('https://app.hyperunit.xyz/withdraw', '_blank')}>
            Withdraw
          </button>
        </div>
      )}
      ...
      {isConnected && state && (
        <div className="tp-unified">
          ...
          <div className="tp-unified-row">
            ...
            <span style={{ color: 'var(--green)' }}>{formatUsd(state.totalBalance)}</span>
          </div>
          <div className="tp-unified-row">
            <span>Unrealized PNL</span>
            <span style={{ color: state.positions.reduce((s, p) => s + parseFloat(p.unrealizedPnl), 0) >= 0 ? 'var(--green)' : 'var(--red)' }}>
